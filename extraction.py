...
class Extraction(abc.ABC):
    ...
    def _tokenize_factory(self, max_length: int = None) -> object:
        """
        A factory function that returns a function used to tokenize text.
        ...
        def tokenize(examples) -> dict:
            ...
            return self.tokenizer(examples["sent"], return_tensors='pt',
    ...
    def _tokens2str(self, tokens: list, tokenizer: object) -> str:
        """
        Convert a list of tokens to a string representation. This function takes in a list of tokens and returns a string
        representation of the tokens, where each token is separated by a space.
    ...
    @abc.abstractmethod
    def extract(self, dataset: str, batch_size: int = 8, max_length: int = 512, layers: int = 12, sampling: int = 0,
                **kwargs) -> object:
        """
    ...
    def extract(self, dataset: str, batch_size: int = 8, max_length: int = 512, layers: int = 12, sampling: int = 0,
        ...
        for i in range(0, tokenized_text.shape[0], batch_size):
            ...
            for j, row in enumerate(batch_tokenized_text):
                ...
                if pos is None:
                    print(f"word_tokens_str: {word_tokens_str},\ninput_tokens_str: {input_tokens_str}")
                    continue
